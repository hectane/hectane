...
func (s *Storage) filename(block string, id int) string {
	return path.Join(s.directory, block, strconv.Itoa(id))
}

// CreateReader attempts to open a reader for the item with the specified ID in
// the specified block.
func (s *Storage) CreateReader(block string, id int) (io.ReadCloser, error) {
	return os.Open(s.filename(block, id))
}

// CreateWriter attempts to open a write for the item with the specified ID in
// the specified block.
func (s *Storage) CreateWriter(block string, id int) (io.WriteCloser, error) {
